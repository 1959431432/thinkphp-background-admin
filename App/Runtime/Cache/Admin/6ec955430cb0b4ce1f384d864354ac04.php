    ...
    <div class="wrapper">
        <div class="bc">
            <ul id="breadcrumbs" class="breadcrumbs">
                 ...
                 <li class=""><a href="<?php echo U('index');?>">管理员分组</a></li>
                 <li class="current"><a href="#">分组权限管理</a></li>
            </ul>
            <div class="clear"></div>
        </div>
        <!-- Validation form -->
        <form class="form validate" method="post" action="<?php echo U('save');?>">
        	...
        	<fieldset>
                <div class="widget">
                    <div class="title"><img src="/Public/Admin/images//icons/dark/alert.png" alt="" class="titleIcon" /><h6>权限表单</h6></div>

                    <div class="formRow">
                        <label>权限选择:</label>
                        <div class="formRight">
                            <?php if(is_array($rules)): $i = 0; $__LIST__ = $rules;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rule): $mod = ($i % 2 );++$i;?><div><span class="checked"><input type="checkbox" name="rules[]" id="rules<?php echo ($rule["id"]); ?>" style="opacity: 0;" value="<?php echo ($rule["id"]); ?>"></span></div>
                                <label for="<?php echo ($rule["id"]); ?>"><?php echo ($rule["title"]); ?></label><?php endforeach; endif; else: echo "" ;endif; ?>
                        </div>
                        <div class="clear"></div>
                    </div>
